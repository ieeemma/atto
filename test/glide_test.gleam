...
pub fn span_char(idx, row, col) {
  glide.Span(glide.Pos(idx, row, col), glide.Pos(idx + 1, row, col + 1))
}

pub fn span_point(idx, row, col) {
  glide.Span(glide.Pos(idx, row, col), glide.Pos(idx, row, col))
}
...
pub fn fail_test() {
  ...
  |> glide.run(text.new(""), Nil)
  |> should.equal(
    Error(glide.ParseError(span_point(0, 1, 1), glide.Msg("foo"), set.new())),
  )

  glide.fail(5)
  |> glide.run(text.new(""), Nil)
  |> should.equal(Error(glide.Custom(span_point(0, 1, 1), 5)))
}

pub fn pos_test() {
  {
    use <- glide.drop(
      glide.pos() |> glide.map(fn(p) { should.equal(p, glide.Pos(0, 1, 1)) }),
    )
    use <- glide.drop(glide.token("a"))
    use <- glide.drop(
      glide.pos() |> glide.map(fn(p) { should.equal(p, glide.Pos(1, 1, 2)) }),
    )
    glide.pure(Nil)
  }
...
pub fn satisfy_test() {
  ...
  |> should.equal(
    Error(glide.ParseError(span_char(0, 1, 1), glide.Token("6"), set.new())),
  )
}
