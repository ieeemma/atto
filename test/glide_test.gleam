...
pub fn single_at(row, col) {
  error.Single(error.Pos(row, col))
}

pub fn span_at(row, col) {
  error.Span(error.Pos(row, col), error.Pos(row, col + 1))
}
...
pub fn fail_test() {
  ...
  |> glide.run(text.new(""), Nil)
  |> should.equal(
    Error(error.ParseError(single_at(1, 1), error.Msg("foo"), set.new())),
  )

  glide.fail(5)
  |> glide.run(text.new(""), Nil)
  |> should.equal(Error(error.Custom(single_at(1, 1), 5)))
}

pub fn pos_test() {
  {
    use <- glide.drop(
      glide.pos() |> glide.map(fn(p) { should.equal(p, error.Pos(1, 1)) }),
    )
    use <- glide.drop(glide.token("a"))
    use <- glide.drop(
      glide.pos() |> glide.map(fn(p) { should.equal(p, error.Pos(1, 2)) }),
    )
    glide.pure(Nil)
  }
...
pub fn satisfy_test() {
  ...
  |> should.equal(
    Error(error.ParseError(span_at(1, 1), error.Token("6"), set.new())),
  )
}
